...
//
// For example, suppose your preamble consists of the numbers 1 through 25 in a random order. To
// be valid, the next number must be the sum of two of those numbers:
//
...
bool canTwoSum(int total, Set<int> numbers, {int depth = 0}) {
  ...
  for (var n in numbers) {
    ...
    var sum_pool = Set<int>.from(numbers);
    sum_pool.remove(n);
    if (canTwoSum(remainder, sum_pool, depth: depth + 1)) return true;
  }

  return false;
}

List<int>? rangeCanSum(List<int> numbers, int total) {
  for (var idx = 2; idx < numbers.length; idx++) {
    var subRange = numbers.sublist(0, idx);
    var sum = subRange.fold<int>(0, (t, n) => t + n);
    if (sum == total) return subRange;
    if (sum > total) break;
  }
}

int solvePartOne(List<int> numbers, int preambleSize) {
  var idx = preambleSize;
  while (idx < numbers.length) {
    var rangeStart = idx - preambleSize;
    var rangeEnd = idx;
    var preamble = Set<int>.from(numbers.sublist(rangeStart, rangeEnd));

    if (!canTwoSum(numbers[idx], preamble)) break;
    idx++;
  }
  return numbers[idx];
}

int solvePartTwo(List<int> numbers, int total) {
  List<int>? sumRange;
  for (var idx = numbers.length - 1; idx >= 0; idx--) {
    var testRange = numbers.sublist(idx, numbers.length - 1);
    sumRange = rangeCanSum(testRange, total);
    if (sumRange != null) break;
  }

  var maxVal = sumRange!.fold<int>(0, (t, n) => n > t ? n : t);
  var minVal = sumRange.fold<int>(maxVal, (t, n) => n < t ? n : t);
  return minVal + maxVal;
}

void main(List<String> arguments) {
  var input =
      File('../input.txt').readAsLinesSync().map((s) => int.parse(s)).toList();

  var partOneSolution = solvePartOne(input, 25);
  print('The solution to part one is $partOneSolution'); // 1212510616

  var partTwoSolution = solvePartTwo(input, partOneSolution);
  print('The solution to part two is $partTwoSolution'); // 171265123
}
