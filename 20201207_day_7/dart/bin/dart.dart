...
// bag is empty, every vibrant plum bag contains 11 bags (5 faded blue and 6 dotted black), and so
// on.
//
...
Map<String, Map<String, int>> readInput(path) {
  final reBagLine = RegExp(r'(^[\w\s]+)(?= bags contain)');
  final reBagCount = RegExp(r'(\d+)\s+([\w\s]+)(?= bag)');
  var lines = File(path).readAsLinesSync().map((s) => s.trim()).toList();
  var output = <String, Map<String, int>>{};

  for (var line in lines) {
    var bagName = reBagLine.firstMatch(line).group(0);
    var contentsMatches = reBagCount.allMatches(line);
    final contents = <String, int>{};

    for (var match in contentsMatches) {
      var containedBagName = match.group(2);
      var containedBagCount = match.group(1);
      if (containedBagName != null && containedBagCount != null) {
        contents[containedBagName] = int.parse(containedBagCount);
      }
    }
    output[bagName] = contents;
  }
...
bool bagContains(
    String bag, String contains, Map<String, Map<String, int>> bagRules) {
  var contents = bagRules[bag];
  ...
  for (var key in contents.keys) {
    ...
    if (bagContains(key, contains, bagRules)) return true;
  }

  return false;
}

int solvePartOne(Map<String, Map<String, int>> bagRules) {
  return bagRules.keys
      .map((key) => bagContains(key, 'shiny gold', bagRules))
      .fold(0, (t, e) => e ? t + 1 : t);
}

int bagCanHold(String bag, Map<String, Map<String, int>> bagRules) {
  var contents = bagRules[bag];
  if (contents == null || contents.isEmpty) {
    return 0;
  }

  var totalBagsContained = 0;
  for (var entry in contents.entries) {
    var result = bagCanHold(entry.key, bagRules);
    totalBagsContained += (result * entry.value) + entry.value;
  }
  return totalBagsContained;
}

int solvePartTwo(Map<String, Map<String, int>> bagRules) {
  return bagCanHold('shiny gold', bagRules);
}
...
void main(List<String> arguments) {
  ...
  print('The answer to part one is $partOneSolution'); // 296

  var partTwoSolution = solvePartTwo(input);
  print('The answer to part two is $partTwoSolution'); // 9339
}
